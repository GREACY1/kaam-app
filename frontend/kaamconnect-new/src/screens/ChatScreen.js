...
export default function ChatScreen({ route }) {
  ...
  const setupChat = async () => {
    ...
    socketRef.current.emit('getMessages', { jobId });

    socketRef.current.on('allMessages', (msgs) => {
      setMessages(msgs);
    });
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={[styles.msg, item.sender === userId ? styles.myMsg : styles.otherMsg]}>
            <Text style={[styles.msgText, item.sender === userId ? styles.myText : styles.otherText]}>
              {item.text}
            </Text>
          </View>
        )}
